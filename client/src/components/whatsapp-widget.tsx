...
// Aynı WhatsApp numarası birden çok sitede kullanıldığı için, bu siteden
// gelen mesajları ayırt etmek üzere gönderilen mesajın başına bu etiketi ekliyoruz.
const SOURCE_TAG = "[Thirdhand AI]";

// API (Netlify Functions) ulaşılamadığında bile widget'ın görünmesi için statik yedek.
const STATIC_FALLBACK: WhatsAppSettings = {
  phoneNumber: "90538961294444",
  welcomeMessage: "",
  enabled: true,
};
...
export default function WhatsAppWidget() {
  ...
  // Statik yedekle başla; API erişilebilirse (functions deploy edilmişse) onunla güncelle
  const [settings, setSettings] = useState<WhatsAppSettings | null>(STATIC_FALLBACK);
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.enabled && data.phoneNumber) {
            setSettings(data);
          }
        }
      } catch (error) {
        // API yoksa (ör. statik Drop deploy) statik yedek kullanılmaya devam eder
        console.error("WhatsApp ayarları yüklenemedi, statik yedek kullanılıyor:", error);
      }
    };
    fetchSettings();
  }, []);
  ...
  const handleWhatsAppClick = () => {
    ...
    // Kaynağı belli olsun diye mesajın başına site etiketini ekle
    const message = encodeURIComponent(`${SOURCE_TAG} ${baseMessage}`);
  ...
  return (
    <div style={{ fontFamily: FONT }}>
      {/* Floating button (sol alt — sağ taraf SesYaz chatbot için ayrıldı) */}
      ...
      {isOpen && (
        <div
          className="fixed bottom-28 left-6 z-50 w-[22rem] max-w-[calc(100vw-3rem)] rounded-3xl overflow-hidden animate-fadeIn"
          style={{
